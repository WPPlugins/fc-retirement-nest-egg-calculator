<?php
// financial-calculators.com retirement nest egg calculator plugin
//
...
// ***********************************************************************
/*
Plugin Name: FC Retirement Nest Egg Calculator
Plugin URI: https://financial-calculators.com/calculator-plugins/retirement-nest-egg-calculator-plugin
...
/*
	...
	Function call:
	<?php show_fcretirenestegg_plugin(<option array>); ?>
	...
	[fcretirenesteggplugin sc_size="medium" sc_custom_style="No" sc_add_link="No" sc_brand_name="" sc_hide_resize="No" sc_current_age="28" sc_retire_age="62" sc_current_savings="16000" sc_invest_amt="750.0" sc_rate="5.5"]

*/


/* Function: activate_fcretirenestegg_plugin
	** activation hook
...
function activate_fcretirenestegg_plugin() {
	...
	update_option('fcretirenesteggcalc_plugin', array(
		'op_size' => null,
		'op_custom_style' => "No",
		'op_add_link' => "No",
		'op_brand_name' => "",
...
register_activation_hook( __FILE__, 'activate_fcretirenestegg_plugin' );


/* Function: show_fcretirenestegg_widget
** Shows the plugin in a WordPress widget area / sidebar
**
** args: $args (environment variables handled automatically by the hook)
** returns: nothing
*/
function show_fcretirenestegg_widget( $args ) {
	extract( $args );
	$options = get_option( 'fcretirenesteggcalc_plugin' );
	$title = null;
	...
	show_fcretirenestegg_plugin($options);

	echo $after_widget;

} // show_fcretirenestegg_widget



/* Function: show_fcretirenestegg_plugin
** Show the plugin's GUI, not in sidebar
**
** args: $options
** returns: nothing
*/
function show_fcretirenestegg_plugin($options = array(), $content = null, $code = "") {

	$shortcode = null;  // The actual shortcode : fcretirenesteggplugin
	...
	if (!empty($code) || (!empty($options) && array_key_exists (0, $options) && (strtolower($options[0]) === 'fcretirenesteggplugin'))){
		$shortcode = true;
	...
	wp_register_script('fc-retirenestegg-interface', plugins_url('js/interface.RETIRE-NESTEGG-WIDGET.min.js', __FILE__), array( 'jquery', 'fc-custom-bootstrap', 'fc-supporting'), '', true);



	wp_enqueue_script( 'fc-retirenestegg-interface' );
...
} // show_fcretirenestegg_plugin



/* Function: fcretirenesteggplugin_options
**
...
function fcretirenesteggplugin_options() {
	...
	$options = $newoptions = get_option('fcretirenesteggcalc_plugin');
	...
	if (!array_key_exists('op_size', $options) || !array_key_exists('op_custom_style', $options) || !array_key_exists('op_add_link', $options) || !array_key_exists('op_brand_name', $options) || !array_key_exists('op_hide_resize', $options) || !array_key_exists('op_current_age', $options) || !array_key_exists('op_retire_age', $options) || !array_key_exists('op_current_savings', $options) || !array_key_exists('op_invest_amt', $options) || !array_key_exists('op_rate', $options)) {
		...
		$options = $newoptions = get_option('fcretirenesteggcalc_plugin'); // keep everything in sync
	}


	// if widget's options have previously been set/saved in current session
	if (!empty($_POST['fcretirenesteggcalc_opts'])) {
		$newoptions['op_size'] =  sanitize_text_field($_POST['fcretirenesteggcalc-op_size']);
		...
		$newoptions['op_custom_style'] =  (strtolower(sanitize_text_field($_POST['fcretirenesteggcalc-op_custom_style'])) === "yes") ? "Yes" : "No";
		$newoptions['op_add_link'] =  (strtolower(sanitize_text_field($_POST['fcretirenesteggcalc-op_add_link'])) === "yes") ? "Yes" : "No";
		// allow word characters, numbers, ampersand, dash, apostrophe, space and number sign
		$newoptions['op_brand_name'] = preg_replace("/[^\w#&'\- ]/", '', $_POST['fcretirenesteggcalc-op_brand_name']);
		if (strtolower($newoptions['op_add_link']) !== 'yes') {
			$newoptions['op_brand_name'] = '';
		}
		$newoptions['op_hide_resize'] =  (strtolower(sanitize_text_field($_POST['fcretirenesteggcalc-op_hide_resize'])) === "yes") ? "Yes" : "No";

		// sanitize and validate numbers
		$newoptions['op_current_age'] =  (is_numeric(sanitize_text_field($_POST['fcretirenesteggcalc-op_current_age']))) ? sanitize_text_field($_POST['fcretirenesteggcalc-op_current_age']) : -1;
		if ($newoptions['op_current_age'] < 0) {
			$newoptions['op_current_age'] = '0';
			echo('Please enter a valid number for "Default current age".'."<br>");
		}

		$newoptions['op_retire_age'] =  (is_numeric(sanitize_text_field($_POST['fcretirenesteggcalc-op_retire_age']))) ? sanitize_text_field($_POST['fcretirenesteggcalc-op_retire_age']) : -1;
		if ($newoptions['op_retire_age'] < 0) {
			$newoptions['op_retire_age'] = '0';
			echo('Please enter a valid number for "Default retirement age".'."<br>");
		}

		$newoptions['op_current_savings'] =  (is_numeric(sanitize_text_field($_POST['fcretirenesteggcalc-op_current_savings']))) ? sanitize_text_field($_POST['fcretirenesteggcalc-op_current_savings']) : -1;
		...
		$newoptions['op_invest_amt'] =  (is_numeric(sanitize_text_field($_POST['fcretirenesteggcalc-op_invest_amt']))) ? sanitize_text_field($_POST['fcretirenesteggcalc-op_invest_amt']) : -1;
		...
		$newoptions['op_rate'] =  (is_numeric(sanitize_text_field($_POST['fcretirenesteggcalc-op_rate']))) ? sanitize_text_field($_POST['fcretirenesteggcalc-op_rate']) : -1;
	...
	if ( $options != $newoptions && (wp_verify_nonce($_POST['fcretirenesteggcalc_opts'], 'fc_options'))) {
		// 2nd check permissions
		if ( is_user_logged_in() && current_user_can('update_plugins') ) {
			$options = $newoptions;
			update_option('fcretirenesteggcalc_plugin', $options);
		}
		//debug
//		else if (!wp_verify_nonce($_POST['fcretirenesteggcalc_opts'], 'fc_options')) {
...
?>
		...
		<p>
			<label for="fcretirenesteggcalc-op_size"><?php _e( 'Widget\'s size?:' ); ?>
				<select name="fcretirenesteggcalc-op_size" id="fcretirenesteggcalc-op_size" class="widefat">
		...
		<p>
			<label for="fcretirenesteggcalc-op_custom_style"><?php _e( 'Load custom style sheet?:' ); ?>
				<select name="fcretirenesteggcalc-op_custom_style" id="fcretirenesteggcalc-op_custom_style" class="widefat">
		...
		<p>
			<label for="fcretirenesteggcalc-op_hide_resize"><?php _e( 'Hide the resize buttons?:' ); ?>
				<select name="fcretirenesteggcalc-op_hide_resize" id="fcretirenesteggcalc-op_hide_resize" class="widefat">
		...
		<p>
			<label for="fcretirenesteggcalc-op_add_link"><?php _e( 'Allow plugin to add links to financial-calculators.com?:' ); ?>
				<select name="fcretirenesteggcalc-op_add_link" id="fcretirenesteggcalc-op_add_link" class="widefat">
		...
		<p><label for="fcretirenesteggcalc-op_brand_name"><?php _e('Add Your Brand Name:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_brand_name" name="fcretirenesteggcalc-op_brand_name" type="text" value="<?php echo $brand_name; ?>" /></label>
		</p>
		<p>
		...
		<div style="width:100%; float:left; clear:both;"><div style="width:45%; float:left; margin-right:4px"><label for="fcretirenesteggcalc-op_current_age"><?php _e('Default current age:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_current_age" name="fcretirenesteggcalc-op_current_age" type="text" value="<?php echo $current_age; ?>" /></label></div>
			<div style="width:45%; float:left"><label for="fcretirenesteggcalc-op_current_savings"><?php _e('Default retirement age:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_retire_age" name="fcretirenesteggcalc-op_retire_age" type="text" value="<?php echo $retire_age; ?>" /></label></div></div>

		<div style="width:100%; float:left; clear:both;"><div style="width:45%; float:left; margin-right:4px;"><label for="fcretirenesteggcalc-op_invest_amt"><?php _e('Default investment amount:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_invest_amt" name="fcretirenesteggcalc-op_invest_amt" type="text" value="<?php echo $invest_amt; ?>" /></label></div>
			<div style="width:45%; float:left"><label for="fcretirenesteggcalc-op_retire_age"><?php _e('Default current savings:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_current_savings" name="fcretirenesteggcalc-op_current_savings" type="text" value="<?php echo $current_savings; ?>" /></label></div></div>
			
			
		<div style="width:100%; float:left; clear:both;"><div style="width:45%; float:left; margin-right:4px;"><label for="fcretirenesteggcalc-op_rate"><?php _e('Default interest rate:'); ?> <input class="widefat" id="fcretirenesteggcalc-op_rate" name="fcretirenesteggcalc-op_rate" type="text" value="<?php echo $rate; ?>" /></label></div><div style="width:45%; float:left"><p style="text-align:center"><?php _e('Enter only digits.<br>No formatting.'); ?></p></div></div>
		...
		<input type="hidden" id="fcretirenesteggcalc_opts" name="fcretirenesteggcalc_opts" value="<?php echo wp_create_nonce('fc_options'); ?>" />
...
/* Function: fcretirenesteggplugin_register
**
...
*/
function fcretirenesteggplugin_register() {
	$widget_ops = array('classname' => 'fcretirenesteggcalc_plugin', 'description' => __('Retirement Calculator by financial-calculators.com'));

	$name = __('Retirement Nest Egg Calculator');
	...
	wp_register_sidebar_widget( 'fcretirenesteggplugin', $name, 'show_fcretirenestegg_widget', $widget_ops );
	...
	wp_register_widget_control( 'fcretirenesteggplugin', $name, 'fcretirenesteggplugin_options' );
} // fcretirenesteggplugin_register


// Hooks a function on to a specific action.
add_action( 'widgets_init', 'fcretirenesteggplugin_register' );

// Adds a hook for a shortcode tag.
add_shortcode( 'fcretirenesteggplugin', 'show_fcretirenestegg_plugin' );
